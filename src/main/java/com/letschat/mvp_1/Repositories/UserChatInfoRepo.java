...
public interface UserChatInfoRepo extends ReactiveCrudRepository<UserChatInfo, Long>{
    ...
    @Query("""
            select uc1."ChatId" as ChatId,ui."UserId" as Id,uc1."ChatName" as ChatName,uc1."Type" as Type,uc1."At" as timestamp from "UserChat" uc1 join "UserChat" uc2 on uc1."ChatId"=uc2."ChatId"
...
select user_chats."ChatId" as ChatId,gi."GroupId" as Id,gi."GroupName" as ChatName,user_chats."Type" as Type,user_chats."At" as timestamp,user_chats."Role" as Role from (select "ChatId","Type","At","Role" from "UserChat" where "UserId"=:UserId )As user_chats join(select "ChatId",MIN("Id") as min_id from "UserChat" group by "ChatId" )
As chat_min_ids on user_chats."ChatId"=chat_min_ids."ChatId" join "GroupInfo" gi on gi."ChatId"=chat_min_ids.min_id            """)
    ...
    @Query("""
select user_chats."ChatId" as ChatId,gi."room_id" as Id,gi."room_name" as ChatName,user_chats."Type" as Type,user_chats."At" as timestamp,user_chats."Role" as Role from (select "ChatId","Type","At","Role" from "UserChat" where "UserId"=:UserId )As user_chats join(select "ChatId",MIN("Id") as min_id from "UserChat" group by "ChatId" )
...
group by "ChatId" having count(distinct "UserId")=2
       ...
       @Query("""
        ...
        @Query("""
                update "UserChat" set "UserName"=:name where "ChatId"=:chatid and "UserId"=:userid
                        """)
        Mono<UserChatInfo> updateUsername(String name,String chatid,String userid);

        @Query("""
                update "UserChat" set "ChatName"=:name where "ChatId"=:chatid and "UserId"!=:userid
                        """)
